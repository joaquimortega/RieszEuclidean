...
a stationary system and two families of orthogonal projections. An exact
bump representation of exponential synthesis transfers the Riesz basis
bound to this system. One family is continuous in operator norm. The geometry of the boundary produces a strict inclusion of closed subspaces. Their associated projections
would both be at operator-norm distance less than one from a third projection, which is a
contradiction.
\end{abstract}
...
The frequencies, translations and spectral measures all remain in
$\R^n$. Sections~\ref{sec:bumps}--\ref{sec:distance} establish the analytic
...
A large language model was used for mathematical discussions.  The results presented
here arose from an initially misguided attempt by the author to disprove
the existence of a Riesz basis of exponentials on a planar ball.  The initial
approach failed and was corrected by the model during these discussions,
leading to the argument presented in this paper.

The standalone Lean formalization of the Euclidean argument is being developed at
\href{https://github.com/joaquimortega/RieszEuclidean}{RieszEuclidean}.
The repository contains the proof blueprint, the Lean scaffold and the
implemented lemmas. Its status report distinguishes the verified components
from the remaining analytic and geometric constructions; the full Euclidean
argument is not yet formally verified.
...
Translations act jointly continuously on this space.
\end{lemma}
...
write $\mu$ for its limit. For $F\in C(X)$ and fixed $y$,
\[
...
of the action on the compact space $X$ makes it strongly continuous on
...
\end{equation}
Uniqueness and the parallelogram identity for the correlations give
...
\[
 ...
 \longrightarrow0.
\]
Products pass to the strong limit for uniformly bounded operators:
\[
 \|\Pi_{t,R}^2f-\Pi_t^2f\|
...
The translation operators themselves are only required to be strongly
...
\begin{remark}[The role of the stationary space]
...
strong-limit and norm-continuity arguments give a contradiction.
